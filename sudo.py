...
class Sudo(dotbot.Plugin):
    ...
    def handle(self, directive, data):
        ...
        try:
            ...
            self._log.debug("sudo: closing fd: {}".format(sudo_conf))
            close(fd)
    ...
    def _collect_plugins(self):
        ...
        for plugin in module.loaded_modules:
            ...
            if plugin.__name__ != self._directive:
                ret.extend(
                    iter(["--plugin", path.splitext(plugin.__file__)[0] + ".py"])
                )
        return ret
    ...
    def _find_dotbot(self):
        self._log.debug("sudo: dotbot app path: {}".format(sys.argv[0]))
        return sys.argv[0]
    ...
    @staticmethod
    def strip_args(args):
        single_args_filter = ["--version", "-h", "--help"]
