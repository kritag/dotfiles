...
local function move_active(ctx, delta)
    ...
    if not i or j < 1 or j > #state.order then return end
...
hl.layout.register("manual", {
    ...
    layout_msg = function(ctx, msg)
        ...
        if command == "splith" or command == "h" then
            if id then state.split[id] = "h" end
        elseif command == "splitv" or command == "v" then
            if id then state.split[id] = "v" end
        elseif command == "splittoggle" or command == "toggle" or command == "togglesplit" then
            if id then state.split[id] = state.split[id] == "v" and "h" or "v" end
